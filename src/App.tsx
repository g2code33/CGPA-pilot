...
export default function App() {
  ...
  const [currentPage, setCurrentPage] = useState<"main" | "target" | "next" | "whatif" | "flight" | "milestones">("main");
  ...
  if (currentPage === 'target') {
    return (
      <div className="min-h-screen bg-gradient-to-b from-brand-50 to-white flex flex-col max-w-5xl mx-auto px-4 py-6 overflow-hidden">
        <header className="flex items-center justify-between mb-6">
          <Brand />
          <button onClick={() => setCurrentPage('main')} className="rounded-xl bg-brand-600 px-4 py-2 text-xs font-black text-white shadow hover:bg-brand-700 transition">◀ Back</button>
        </header>
        <main className="flex-1 bg-white rounded-3xl shadow-xl shadow-brand-900/5 ring-1 ring-brand-100 p-6 overflow-hidden">
          <Target />
        </main>
        <div className="flex gap-3 mt-4">
          <button onClick={() => setCurrentPage('target')} className="flex-1 rounded-2xl bg-brand-600 px-6 py-4 text-base font-black text-white shadow hover:bg-brand-700 active:scale-[0.98] transition text-center">🎯 Target</button>
          <button onClick={() => setCurrentPage('next')} className="flex-1 rounded-2xl bg-emerald-600 px-6 py-4 text-base font-black text-white shadow hover:bg-emerald-700 active:scale-[0.98] transition text-center">▶️ Next Semester</button>
          <button onClick={() => setCurrentPage('whatif')} className="flex-1 rounded-2xl bg-violet-600 px-6 py-4 text-base font-black text-white shadow hover:bg-violet-700 active:scale-[0.98] transition text-center">🔀 What-If</button>
        </div>
      </div>
    );
  }
  ...
  if (currentPage === 'whatif') {
    return (
      <div className="min-h-screen bg-gradient-to-b from-brand-50 to-white flex flex-col max-w-5xl mx-auto px-4 py-6 overflow-hidden">
        ...
        <main className="flex-1 bg-white rounded-3xl shadow-xl shadow-brand-900/5 ring-1 ring-brand-100 p-4 overflow-hidden">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-2xl">💡</span>
            <h2 className="text-xl font-black text-slate-900">What-If · Flight · Milestones</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="bg-violet-50 rounded-2xl p-3 ring-1 ring-violet-100 overflow-hidden">
              <h3 className="text-xs font-black uppercase tracking-wide text-violet-700 mb-2">What-If</h3>
              <WhatIf />
            </div>
            <div className="bg-brand-50 rounded-2xl p-3 ring-1 ring-brand-100 overflow-hidden">
              <h3 className="text-xs font-black uppercase tracking-wide text-brand-700 mb-2">Flight Path</h3>
              <FlightPathView />
            </div>
            <div className="bg-emerald-50 rounded-2xl p-3 ring-1 ring-emerald-100 overflow-hidden">
              <h3 className="text-xs font-black uppercase tracking-wide text-emerald-700 mb-2">Milestones</h3>
              <Milestones />
            </div>
          </div>
        </main>
