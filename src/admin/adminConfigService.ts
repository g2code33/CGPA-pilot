...
// ── Review validation + offline distribution ────────────────────────────
// The publish gate (reviewCurriculum / canPublish) and the distribution
// payload builders (buildDistribution / isDistributionPayload) live in
// ./catalogPublish — pure, shared by the admin console, the tests and the
// Cloudflare Worker. Re-exported here so existing imports keep working.
export * from './catalogPublish';
import { canPublish, reviewCurriculum } from './catalogPublish';
