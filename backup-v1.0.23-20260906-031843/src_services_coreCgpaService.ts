...
export function currentModeRecord(
  ...
  let totalCompleted = fromCurriculum
    ? (curriculumCompletedCredits as number)
    : baseline.creditHours || 0;
  // Just entered this level/semester: calculations use completed work up to
  // the immediate past semester (exclude the current selection).
  if (baseline.justEntered && fromCurriculum && configuredCreditsFor) {
    const currentSemesterCredits = configuredCreditsFor(
      baseline.levelIndex,
      baseline.semesterIndex
    );
